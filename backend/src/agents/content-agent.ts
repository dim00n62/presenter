...
### Bullet Points
- Заголовок: четкое утверждение
- 3-7 пунктов
- Каждый пункт начинается с заглавной
- Иерархия: основные → подпункты
...
1. ❌ НИКОГДА не добавляйте специальные маркеры в текст:
   - НЕ используйте [HIGHLIGHT]
   - НЕ используйте [ICON: ...]
   - НЕ используйте [COLOR: ...]
   - НЕ используйте [Источник: ...] внутри текста
   
2. ✅ Визуальные подсказки ТОЛЬКО в visualHints
3. ✅ Источники ТОЛЬКО в поле sources (массив)
4. ✅ Чистый текст без служебных символов
5. ✅ Footer может содержать краткую ссылку, но НЕ полный путь к файлу

ПРАВИЛЬНО:
"bullets": ["Oracle 11g (EOL с 2020 года)", "2 ТБ данных"]

НЕПРАВИЛЬНО:
"bullets": ["Oracle 11g (EOL с 2020 года) [HIGHLIGHT]", "2 ТБ данных [ICON: database]"]
